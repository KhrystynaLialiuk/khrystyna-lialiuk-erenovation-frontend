...
import com.kodilla.erenovation.dto.response.ReservationAddressDto;
import com.kodilla.erenovation.dto.response.ReservationDto;
...
public class ReservationAddressFrom extends FormLayout {
    ...
    private void createNewReservation(ReservationDto reservationDto) {
        HttpStatus status = eRenovationClient.postReservation(reservationDto);
        if (status.value() == 201) {
    ...
    private void updateExistingReservation(ReservationDto reservationDto) {
        eRenovationClient.putReservation(reservationDto);
        clear();
    }
}
